...
var keytask = {
    enter: (event) => {//Title Scene
        if (getState() == 'home') {
            document.querySelector('.button--dance').click();
        }
        if (getState() == 'start') {
    ...
    arrowLeft: (event) => {
        ...
        if (getState() == 'hud' && gamevar.isPaused) {
    ...
    F1: (event) => {
        if (getState() == 'home') {
            ...
            globalfunc.startTransition(true, 'scene/ui/home.html', 'scene/act/home.js')
        }
    },
    ESC: (event) => {
        if (getState() == 'songselection') {
            event.preventDefault()
            globalfunc.startTransition(true, 'scene/ui/title.html', 'scene/act/title.js')
        }
        if (getState() == 'hud') {
            event.preventDefault()
